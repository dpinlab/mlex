...
from mlex import DataReader, RNN
...
def main():
    ...
    for sequence_composition in compositions:
        ...
        for seq_len in seq_lens:
            ...
            output_dir = join('RESULTS_TSNE', 'RNN', str(seq_len), str(sequence_composition))
            makedirs(output_dir, exist_ok=True)
            
            for exec_ in range(1, total_runs+1):
                print(f"Initializing RNN model (Seq: {seq_len}, Comp: {sequence_composition}, Run: {exec_})...")
                model_RNN = RNN(
                    target_column='I-d',
                ...
                _ = model_RNN.predict_proba(X_test)

                print("Extracting activations...")

                predict_activations = model_RNN.final_model.activations['predict']
                ...
                activation_indices = model_RNN.activation_indices_
                ...
                # Main super title
                plt.suptitle(f'RNN Analysis - Context: {sequence_composition} - Seq_len: {seq_len}', fontsize=16)
                ...
                plt.title('t-SNE of RNN Last Hidden State\n(Colored by Ground Truth Labels)')
                plt.xlabel('t-SNE 1')
                plt.ylabel('t-SNE 2')
    ...
    if all_runs_data:
        ...
        parquet_path = join('tsne-results', 'rnn_experiment_results_all.parquet')
