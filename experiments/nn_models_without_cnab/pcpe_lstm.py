...
with mlflow.start_run(run_name="LSTM_without_CNAB_v1"):
    ...
    model_LSTM = LSTM(validation_data=validation_data, target_column='I-d', categories=categories, device=device, categorical_features=['TIPO', 'NATUREZA_SALDO'], numeric_features=['DIA_LANCAMENTO','MES_LANCAMENTO','VALOR_TRANSACAO','VALOR_SALDO'], context_feature=['GROUP'])
    ...
    mlflow.log_dict(make_json_serializable({k: v for k, v in model_LSTM.get_params().items() if k != "validation_data"}), "model_params.json")
    mlflow.log_dict({'train_data': 'pcpe_03.csv', 'test_data': 'pcpe_04.csv', 'context':'baseline'}, "experimental_setup.json")
    mlflow.sklearn.log_model(model_LSTM, name="model", registered_model_name=MODEL_NAME)
