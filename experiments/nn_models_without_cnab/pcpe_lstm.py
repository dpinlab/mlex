...
with mlflow.start_run(run_name="LSTM_without_CNAB_v1"):
    ...
    y_pred_score = model_LSTM.predict_proba(X_test)

    model_LSTM.set_params(threshold = threshold_selection.compute_threshold(y_test, y_pred_score))

    evaluator = StandardEvaluator(f"LSTM_pipeline", threshold=model_LSTM.threshold)
