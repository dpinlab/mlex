...
import numpy as np
from mlex import RNN, DataReader, F1MaxThresholdStrategy, StandardEvaluator, make_json_serializable
...
with mlflow.start_run(run_name="RNN_without_CNAB_v1"):
    ...
    y_pred_score = model_RNN.predict_proba(X_test)

    model_RNN.set_params(threshold = threshold_selection.compute_threshold(y_test, y_pred_score))

    evaluator = StandardEvaluator(f"RNN_pipeline", threshold=model_RNN.threshold)
    ...
    mlflow.log_dict(make_json_serializable({k: v for k, v in model_RNN.get_params().items()}), "model_params.json")
    mlflow.log_dict({'train_data': 'pcpe_03.csv', 'test_data': 'pcpe_04.csv', 'context':'baseline'}, "experimental_setup.json")
    mlflowsklearn.log_model(model_RNN, name="model", registered_model_name=MODEL_NAME)
