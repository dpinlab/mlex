...
from mlex import DataReader, BILSTM
...
def main():
    ...
    for sequence_composition in compositions:
        ...
        for seq_len in seq_lens:
            ...
            output_dir = join('RESULTS_TSNE', 'BILSTM', str(seq_len), str(sequence_composition))
            makedirs(output_dir, exist_ok=True)
            
            for exec_ in range(1, total_runs+1):
                print(f"Initializing BILSTM model (Seq: {seq_len}, Comp: {sequence_composition}, Run: {exec_})...")
                model_BILSTM = BILSTM(
                    target_column='I-d',
                ...
                _ = model_BILSTM.predict_proba(X_test)

                print("Extracting activations...")

                predict_activations = model_BILSTM.final_model.activations['predict']
                ...
                activation_indices = model_BILSTM.activation_indices_
                ...
                # Main super title
                plt.suptitle(f'BILSTM Analysis - Context: {sequence_composition} - Seq_len: {seq_len}', fontsize=16)
                ...
                plt.title('t-SNE of BILSTM Last Hidden State\n(Colored by Ground Truth Labels)')
                plt.xlabel('t-SNE 1')
                plt.ylabel('t-SNE 2')
    ...
    if all_runs_data:
        ...
        parquet_path = join('tsne-results', 'bilstm_experiment_results_all.parquet')
