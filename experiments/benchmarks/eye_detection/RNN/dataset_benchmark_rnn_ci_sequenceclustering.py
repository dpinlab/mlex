...
for sequence_length in sequence_lengths:
    ...
    for exp_name in experiment_names:
        if exp_name == 'temporal':
            ...
            train_path = f'{base_path}_{cluster_name}_Seq{sequence_length}_train.csv'
            test_path = f'{base_path}_{cluster_name}_Seq{sequence_length}_test.csv'
            sequence_composition = 'feature'

    
        print(f"\n### Rodando experimento: {exp_name} (Seq={sequence_length}, Comp: {sequence_composition}) ###") 
        ...
        for i in range(iterations):
            ...
            evaluator = StandardEvaluator(f"RNN_Layers-{num_layers}_HiddenSize-{hidden_size}_SequenceLength-{sequence_length}_{sequence_composition}_{threshold_strategy}_Iteration-{i+1}_{exp_name}", threshold_selection)
