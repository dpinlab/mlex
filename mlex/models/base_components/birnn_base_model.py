...
class BIRNNBaseModel(nn.Module):
    def __init__(
        self,
        validation_data,
        input_size=None,
        ...
        **kwargs
        ...
        linear_input_size = self.hidden_size * 2
        self.linear = nn.Linear(linear_input_size, self.output_size)
    ...
    def __forward(self, x):
        # BIRNN forward pass
        # birnn_out: (batch_size, seq_length, hidden_size)
        # hidden: (num_layers, batch_size, hidden_size)
        birnn_out, hidden = self.birnn(x)

        # Take the output from the last time step
        # rnn_out[:, -1, :] has shape (batch_size, hidden_size)
        last_output = birnn_out[:, -1, :]
